...
Прежде чем продолжить — небольшой дисклеймер — я не имею прямого отношение ни к одной упоминающейся ниже компании, рекламной нагрузки этот чисто технический пост не несёт.
...
Я в курсе про то, что Ютьюб в России нынче работает с перебоями, поэтому скачал его для вас и переложил на Яндекс Диск, с которым проблем быть не должно: [Полное видео (200МБ)](https://disk.yandex.com/i/0dw_Yv1gWfj9UA), [одна минута с 2:55 (10 МБ)](https://disk.yandex.com/i/ccSjt0KhqoX-qg), и ещё [полминутки с 5:23 (5МБ)](https://disk.yandex.com/i/uUojflUIv8vN9g)., тут ещё и рекламы не будет.
...
Теперь вы знаете почти всё. Дотошный зритель наверняка задаст себе вопрос "куда-то это всё хозяйство должно быть подключено?". Рабочее напряжение там 24 вольта, что для датчиков, что для исполнительных механизмов, это стандарт, принятый в промышленности. А подключается всё это к ПЛК, который мы и рассмотрим ниже.
...
Впрочем нет, я могу "снять" крышку вот так:
...
Пустой слот посередине между терминальным блоком и процессором предназначен для модулей расширения [других шин](https://www.br-automation.com/en-us/products/networks-and-fieldbus-modules/) типа, Profibus, ModbusTCP, DeviceNet и т.д., например если вам нужно общаться с другой железкой по протоколу CanOpen, то вы вставите туда  модуль [X20IF1041-1](https://www.br-automation.com/en/products/io-systems/x20-system/x20-interface-module-communication/x20if1041-1/), вот так:
...
Электроника как электроника. Продвинутые электронщики возможно разглядят тут какие-нибудь мосфеты или опторазвязки, но я не специалист, моя схемотехника закончилась на операционниках да самодельных усилителях звука.
...
Показать содержимое шкафа из продакшена я не могу, но для обучения юных падаванов наши практиканты собрали такой вот кубик, тут нет ничего секретного — пара моторов, стандартный B&R монитор для SCADA/HMI, безопасный ПЛК в комплекте, кнопки да индикаторы, больше в общем ничего не надо:
...
Ну вот, теперь. после несложных упражнений можно перейти к делу. Я, кстати, безотносительно ПЛК рекомендую всем написать несколько несложных функций на ассемблере, тут вы прокачаете и поймёте соглашения о вызовах, да и процессор перестанет быть магией.
...
Кстати, загрузку процессора можно посмотреть в "Таск менеджере", который суть веб сервер, и мы всегда можем зайти на ПЛК из браузера:
