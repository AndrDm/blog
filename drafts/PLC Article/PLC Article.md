...
Компания B&R была основана в 1979 году Эрвином Бекером и Джозефом Райнером, откуда и название которое иногда пишут как B+R. Штаб квартира и основное производство с учебными центрами располагаются в австрийском Эггельсберге:
...
Говорят, его название сменили на Fugging, но я проезжал там несколько раз в 2019 году, когда ездил на обучение и застал именно ту вывеску. Вот за тем лесом, километров десять и будет Эггельсберг. В настоящее время компания приадлежит концерну АВВ, который известен своими роботами.
...
Тут мой коллега, поглядев на мои эксперименты, заявил, что мол как-же так - ПЛК - это типа эталон надёжности, а ты его на Си, можно ж "выстрелить себе в ногу" как это нынче модно говорить. Понятное дело, что ассемблерными вставками я могу отстрелить себе вообше всё что угодно, но давайте попробуем, нет проблем:
...
этих двух операторов достаточно, чтобы положить ПЛК "на лопатки". При исполнении этого кода он мгновенно перезагрузится, и перейдёт в сервисный режим. Совсем он не умрёт, веб сервер останется в строю, и там можно увидеть статус:
...
Другой пример, я положу программу для вычисления SHA256 в цикл 10 мс. Поскольку для вычисления нужно раз в десять больше времени, то мы гарантированно спровоцируем таймаут.

Результат, как говрится "те же, там же" - мы снова встали в сервис, вот только причина другая:
...
И так далее. В принципе "окирпичить" этот ПЛК непросто, так как даже если программа окажется настолько кривой, что всё встанет вообще, то всегда можно просто вытащить флешку, закинуть на неё "живую" программу, прошить в кардридере и ПЛК будет как новенький, снова в в строю.

### Идём в Плюсплюсы

---

Компиляция Си и С++ программ осуществляется при помощи тулчейна gcc. Предыдущие версии Automation Studio включали в себя древнейшие версии начала века, там просто не поддержвались современные стандарты С++. Мы даже обращалис0 в поддержку, и вот оно, свершилось - в Automation Studio 6 завезли gcc v. <версию проверь>. Мир заиграл новыми красками.

Я это к чему. Один из основных паттернов проектирования в АСУ ТП - это конечный автомат. Если бы я проводил собеседование, то непременно спросил бы, чем отличаются автоматы Мили и Мура, нарисовать RS триггер, а самых продвинутых попросил бы изобразить конечный автомат, перемножающий два двоичных числа.

Если вы программируете на ST, то скорее всего вас ждёт развесистый switch, каждый кейс которого - суть состояние. В несложный автоматах это норм, но при усложнении будут проблемы.

Наш ведущий инженер сделал универсальную библиотечку для конечных автоматов используя ООП, но во во время проектирования он испытывал сильные моральные страдания, будучи вынужден пользоваться устаревшим инструментом (пытаться подменять тулчейн смысла нет, там gcc допиленный B&R). Выкладывать код с работы я по понятным причинам не имею права, но на С++ существуют готовые открытые решения разной степени удобства и элегантности. Я взял одно из них, вот отсюда. Это реализация довольно удобная, в одном заголовочном файле, и полностью совместима с последней версией Cтудии.

+ rjvgbkywby bp rkvfzlzjq cnhjrb
